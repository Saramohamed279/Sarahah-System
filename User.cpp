...
void User::addContact(list<SarahaUsers>& users, list<Contact>& contacts)
{
	...
	if (contactFound)
	{
		cout << "Contact add to your contact list." << endl;
		FileManager::setContact(contacts);
		cout << "New contacts added." << endl;
	}
...
void User::userOptions(list<SarahaUsers>& users, list<Contact>& contacts, stack<Message>& messages, queue<Message>& favoriteMessages)
{
	...
	while (choice != 12) {
		...
		case 7:
			system("cls");
			viewReceivedMessages(messages, contacts);
			break;
		case 8:
			system("cls");
...
void User::viewReceivedMessages(stack<Message> messages, list<Contact>contacts) {
